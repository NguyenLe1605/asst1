...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  __cs149_mask maskAll, maskZero, maskNotZero, maskPositive, maskMax;

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    int width = VECTOR_WIDTH;
    if (N % width != 0 && i + VECTOR_WIDTH >= N) {
    ...
    // based on the predicate set output[i] to 1.0f
    _cs149_vset_float(result, 1.0f, maskZero);

    // the else case
    maskNotZero = _cs149_mask_not(maskZero);

    // load the result
    _cs149_vmove_float(result, value, maskNotZero);

    // initalize the count;
    _cs149_vmove_int(count, exp, maskNotZero);
    _cs149_vsub_int(count, count, one, maskNotZero);

    // Do the exponent
    _cs149_vgt_int(maskPositive, count, zero, maskNotZero);

    while (_cs149_cntbits(maskPositive) > 0) {
      _cs149_vmult_float(result, result, value, maskPositive);
      _cs149_vsub_int(count, count, one, maskPositive);
      _cs149_vgt_int(maskPositive, count, zero, maskPositive);
    }

    // clamp the vector
    maskMax = _cs149_mask_and(maskAll, maskNotZero);
    _cs149_vgt_float(maskMax, result, max, maskNotZero);
...
float arraySumVector(float *values, int N) {
  ...
  _cs149_vset_float(result, 0.0f, maskAll);
  int counter = 1;
